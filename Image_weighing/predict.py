...
def predict(image_dir):
    fix_img_dir = './duck2/' + image_dir +'/'
    image = cv2.imread(fix_img_dir)
...
def main():
    ...
    while True:
        image_dir = input("Please enter the code of the duck you want to predict (enter the number '0' to end the program)")
        if image_dir == '0':
            break
        try:
            predict_data = predict(image_dir)
            feed_dict={X:predict_data, keep_prob:1}
            print("The duck is expected to weigh："+ str(sess.run(result, feed_dict)[0][0])+ 'kg')
        except:
